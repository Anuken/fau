...
proc initGraphics*() =
  ...
  fuse.batchFlush = proc() = 
    raise Exception.newException("Graphics state is not initialized.")
  fuse.batchDraw = proc(region: Patch, x: float32, y: float32, width: float32, height: float32, originX: float32 = 0, originY: float32 = 0, rotation: float32 = 0, color: float32 = 0) = 
    raise Exception.newException("Graphics state is not initialized.")
